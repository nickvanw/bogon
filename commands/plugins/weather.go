...
func weatherLookup(msg commands.Message, ret commands.MessageFunc) string {
	f, err := fetchWeatherFromLoc(msg.Params[1:])
	if err != nil {
		return err.Error()
	}
	dirs := []string{"N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE", "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW", "N"}
	pos := int(((f.Currently.WindBearing + 11.25) / 22.5))

	out := fmt.Sprintf("%s: is %s - %.0fF (feels: %.0fF) with %.0f%% humidity | wind: %.0f mph %s",
		f.Location, f.Currently.Summary, f.Currently.Temperature, f.Currently.ApparentTemperature,
		f.Currently.Humidity*100.0, f.Currently.WindSpeed, dirs[pos])

	if f.Currently.PrecipIntensity != 0 {
		out = fmt.Sprintf("%s | precip: %s (%.2f in/hr)", out, f.Currently.PrecipType, f.Currently.PrecipIntensity)
	}
	if f.Minutely.Summary != "" {
		out = fmt.Sprintf("%s | soon: %s", out, f.Minutely.Summary)
	}
...
func fetchWeatherFromLoc(loc []string) (*forecast, error) {
	geoAddr, err := util.GetCoordinates(loc)
	if err != nil {
		return nil, err
	...
	f.Location = geoAddr.FormattedAddress
	return f, nil
}
